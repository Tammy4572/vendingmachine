     ...
     if(!item){
          ...
          if(item.cost > moneyReceived) {
               ...
               var purchase = {
                    ...
                    price: item.cost,
                    date: moment().format("dddd, MMMMM Do YYY, h:mm a")
               }
...
application.post('/api/vendor/items', function( request, response) {
     ...
     if (description && cost && quantity) {
          ...
          return.json(modelStat);
     }
});

application.put('/api/vendor/items/:itemId', function ( request, response) {
     var itemId = parseInt(requst.params.itemId);
     var description = request.body.description;
     var cost = requst.body.cost;
     var quantity = request.body.quantity;
     var updatedItem = {
          description: description,
          cost: cost,
